...
- (void)viewWillDisappear:(BOOL)animated
{
    ...
    if(!_isGoNext)
    {
        ((TTableViewController*)_lastViewController).isGoNext = NO;
        if(_block_viewWillGoBack)
        {
            _block_viewWillGoBack();
        }
        if(_lastViewController && ((TTableViewController*)_lastViewController).block_viewGoBackCallback)
        {
            ((TTableViewController*)_lastViewController).block_viewGoBackCallback(_object_viewGoBackCallback);
        }
    }
}
